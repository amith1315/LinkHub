...
**LinkHub** is a minimalist productivity tool designed to organize project-related links (documents, spreadsheets, slides, drives, repositories, Figma specs, and Notion nodes) into clean, customizable folder tiles. 
...
- **Recovery Codes**: Get a masked 6-digit recovery key (`XXXXXX`) on your dashboard to restore your projects and settings on other browsers or devices.
- **Project Sharing**: Click the share icon next to a project title to copy a share URL (e.g. `#/share/:projectId`). Opening this link clones the project and its links directly into another user's workspace.
- **Project Folders & Descriptions**: Organize your workspaces in clean square grid cards. Add optional descriptions to annotate your project directories.
- **Persistent Drag-and-Drop Sorting**: Drag and reorder both your project folder cards and saved link cards. Swapping lists updates a database `sort_order` sequence index coordinate persistently so your custom folder and link order is saved across sessions.
...
- **Link Tiles**: Saved links display as square tiles. Hovering over a tile reveals action triggers (Copy Link, Open Tab, Edit, Delete). The content elements translate upward smoothly on hover to avoid overlap with action buttons. Clicking anywhere on the tile opens the target URL.
- **Light/Dark Themes**: Fully persistent Light and Dark themes, optimized for modern visual aesthetics, with monochrome welcome splash gradients.
---
...
- **Database**: PostgreSQL (Supabase) with an automated JSON file fallback for local offline development.
