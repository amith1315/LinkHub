...
- **Branded Microsoft Teams Icon**: Detects `teams.microsoft.com` / `teams.live.com` links and renders an inline branded purple Teams logo card.
- **Interactive Square Link Tiles**: Saved links display as square tiles. Hovering over a tile reveals action triggers (Copy Link, Open Tab, Edit, Delete). The content elements translate upward smoothly on hover to avoid overlap with action buttons. Clicking anywhere on the tile opens the target URL.
...
- **Logout Safety Warning**: Double-checks with a confirmation popup before logging out to prevent losing access to recovery keys.
